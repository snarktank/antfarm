...
describe("buildPollingPrompt", () => {
  ...
  it("works with different workflow/agent ids", () => {
    const prompt = buildPollingPrompt("bug-fix", "fixer", MODEL);
  ...
  it("uses the provided model in the spawned task", () => {
    const prompt = buildPollingPrompt("feature-dev", "developer", MODEL);
    assert.ok(prompt.includes(`"${MODEL}"`), "should use provided model");
  });
