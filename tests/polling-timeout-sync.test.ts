...
describe("polling timeout sync across all workflows", () => {
  for (const name of WORKFLOW_NAMES) {
    ...
    it(`${name} workflow polling.model is set to 'google-gemini-cli/gemini-3-flash-preview' (OpenClaw resolves model)`, async () => {
      const dir = path.join(WORKFLOWS_DIR, name);
      const spec = await loadWorkflowSpec(dir);
      ...
      assert.equal(
        ...
        `${name} polling model should be "google-gemini-cli/gemini-3-flash-preview" to let OpenClaw resolve the model, got: ${spec.polling.model}`
      );
    });
  }
